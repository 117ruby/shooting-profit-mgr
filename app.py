...
def create_pdf(df, sales, cost, profit, adj_time):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 20); pdf.set_text_color(0, 104, 201)
    pdf.cell(0, 15, "rubyohoto PROFIT REPORT", ln=True, align='C'); pdf.ln(5)
    ...
    pdf.cell(0, 12, f"TOTAL PROFIT: {int(profit):+,}", ln=True, align='R'); pdf.ln(5)
